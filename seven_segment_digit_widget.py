...
class One7SegmentDigit(tk.Frame):
    """Egy olyan grafikus elemet (widget) valósít meg, amely a render_digit() metódusnak str vagy int típusú
    argumentumként átadott számjegyet hétszegmenses számjegy formájában rajzolja ki.
    """
    ...
    def render_digit(self, num: int | str):
        ...
        num_segments_to_be_displayed = {'0': self.all_segment_tags - {SegmentId.CENTER.name},
                                        ...
                                        '2': self.all_segment_tags - {SegmentId.TOP_LEFT.name,
                                                                      SegmentId.BOTTOM_RIGHT.name},
                                        '3': self.all_segment_tags - {SegmentId.TOP_LEFT.name,
                                                                      SegmentId.BOTTOM_LEFT.name},
                                        '4': {SegmentId.TOP_LEFT.name, SegmentId.TOP_RIGHT.name, SegmentId.CENTER.name,
                                              SegmentId.BOTTOM_RIGHT.name},
                                        '5': self.all_segment_tags - {SegmentId.TOP_RIGHT.name,
                                                                      SegmentId.BOTTOM_LEFT.name},
                                        '6': self.all_segment_tags - {SegmentId.TOP_RIGHT.name},
                                        '7': {SegmentId.TOP.name, SegmentId.TOP_RIGHT.name,
                                              SegmentId.BOTTOM_RIGHT.name},
...
if __name__ == '__main__':
    ...
    digit_seq = Digits7Segments(root, '1234567890', 60, segment_color='red',
                                segmentsdefinitions_class=model.Digit1SegmentDefinitions)
